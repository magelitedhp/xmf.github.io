...
[动态更新] 当前稳定音乐源：netease、joox、bilibili

[动态更新] 当前访问频率限制：5分钟内不超50次请求
...
count：页面长度。选填，一次返回显示多少内容，默认为20条
...
id：曲目ID。必填，即track_id，根据音乐源不同，曲目ID的获取方式各不相同，可通过本站提供的搜索接口获取

br：音质。选填，可选128、192、320、740、999（默认），其中740为16bit无损，999为24bit无损
...
id：专辑图ID。必填，专辑图ID即pic_id，可通过本站提供的搜索接口获取

size：图片尺寸。选填，可选300（默认）、500，其中300为小图，500为大图，返回的图片不一定是300px或500px
...
id：歌词ID。必填，歌词ID即lyric_id（一般与曲目ID相同），可通过本站提供的搜索接口获取
